# -*- coding: utf-8 -*-
"""
Created on Mon Feb  4 23:50:33 2019
...
for i in range(xtrainlen):
    ...
    kp[i], des[i] = sift[i].detectAndCompute(gray[i],None)
...
for features in des:
    ...
    for feature in features:    
        temp.append(feature/np.linalg.norm(feature))
    normdes.append(temp)
...
for i in range(1,xtrainlen):
    X_train = np.append(X_train,normdes[i],axis=0)
...
for i in range(xtestlen):
    graytest[i] = cv2.cvtColor(test[i],cv2.COLOR_BGR2GRAY)
    sifttest[i] = cv2.xfeatures2d.SIFT_create()
    kptest[i], destest[i] = sifttest[i].detectAndCompute(graytest[i],None)

#normalizing surf features
normdestest=[]
for features in destest:
    temp=[]
    for feature in features:    
        temp.append(feature/np.linalg.norm(feature))
    normdestest.append(temp)
...
for i in range(1,xtestlen):
    X_test = np.append(X_test,normdestest[i],axis=0)
...
for i in range(xtestlen):
    lengths_out.append(len(destest[i]))
...
destination='sift+surf/k='+str(clust)
