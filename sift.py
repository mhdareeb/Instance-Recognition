# -*- coding: utf-8 -*-
"""
Created on Mon Feb  4 23:41:22 2019
...
for i in range(1,xtrainlen):
    X_train = np.append(X_train,des[i],axis=0)
...
for i in range(1,xtestlen):
    X_test = np.append(X_test,destest[i],axis=0)
...
y_train = np.loadtxt('y_kmeans_10000_sift.txt',dtype='float32')
y_test = np.loadtxt('y_test_10000_sift.txt',dtype='float32')
...
destination='only sift/k='+str(clust)
